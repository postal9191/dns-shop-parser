...
def _terminate_process(process: asyncio.subprocess.Process) -> None:
    ...
    try:
        ...
        logger.debug("[QRATOR] Failed to terminate Node process gracefully: %s", exc)
...
def _kill_process(process: asyncio.subprocess.Process) -> None:
    ...
    try:
        ...
        logger.debug("[QRATOR] Failed to kill Node process: %s", exc)
...
async def resolve_qrator_cookies(user_agent: str | None = None, retry_count: int = 0) -> dict[str, str] | None:
    ...
    try:
        ...
        logger.debug("[QRATOR] Full error:", exc_info=True)
        return None
