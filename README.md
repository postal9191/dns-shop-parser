# DNS Shop Parser Уценённые товары

**Версия 1.0** — ✅ Рабочая на Windows и Linux  
Автоматический парсер товаров с сайта DNS-shop.ru (категория уценка)
...
```env
# Куки города (ГЛАВНЫЕ параметры, они определяют регион парсинга)
CITY_COOKIE_PATH=moscow              # Название города в URL
CITY_COOKIE_CURRENT=c5f58b981d1ed0bad...  # Хеш SHA256 данных о городе (обрезано для примера)

# ID города (нужен только если менять город, куки строятся автоматически)
CITY_ID=884019c7-cf52-1.....
CITY_NAME=Москва
...
TELEGRAM_TOKEN=                 # Оставить пусто или указать токен бота
TELEGRAM_CHAT_ID=               # ID чата для уведомлений
```

**⚠️ Важно:** Главные параметры для парсинга конкретного города — это `CITY_COOKIE_PATH` и `CITY_COOKIE_CURRENT`. 
Они устанавливают регион DNS Shop, из которого будут приходить товары.
...
**Важно:** DNS Shop возвращает куки для Москвы по умолчанию. Эти два параметра определяют, из какого города приходят товары:

| Параметр | Описание |
|----------|---------|
| `CITY_COOKIE_PATH` | Название города в URL (например, `krasnodar`, `moscow`) |
| `CITY_COOKIE_CURRENT` | Хеш SHA256 данных о городе (устанавливает регион в DNS Shop API) |

Эти куки сохраняются в `.env` и используются при каждом запросе к API:
```env
CITY_COOKIE_PATH=krasnodar
CITY_COOKIE_CURRENT=c5f58b981d1ed0bad...  # Пример (обрезано)
```
...
Для парсинга других городов нужно обновить **главные параметры кук**:

1. Найти `CITY_COOKIE_PATH` и `CITY_COOKIE_CURRENT` для нужного города
   - `CITY_COOKIE_PATH` — название города в URL (например, `krasnodar`, `spb`, `ekaterinburg`)
   - `CITY_COOKIE_CURRENT` — хеш SHA256, можно получить запросом к API DNS Shop с нужным городом
2. Обнови `.env`:
   ```env
   CITY_COOKIE_PATH=<город>
   CITY_COOKIE_CURRENT=<хеш>
   
   # Опционально (для справки):
   CITY_ID=<ID города>
   CITY_NAME=<Название города>
   ```
3. Удали `dns_monitor.db` чтобы начать парсинг с чистой БД
4. Запусти `python parser.py` для проверки новых кук
5. Запусти `python run.py` для начала постоянного парсинга
...
### Город неправильный
**Главные параметры, определяющие регион парсинга:**
- `CITY_COOKIE_PATH` — название города в URL (например, `krasnodar`, `moscow`)
- `CITY_COOKIE_CURRENT` — хеш SHA256 данных о городе (устанавливает регион в DNS Shop)

**Если приходят товары неправильного города:**
1. Проверь что `CITY_COOKIE_PATH` и `CITY_COOKIE_CURRENT` правильные для нужного города
2. Если нужен другой город, обнови оба параметра в `.env`
3. Удали `dns_monitor.db` чтобы начать парсинг с чистой БД
4. Запусти `python parser.py` для переполучения кук и товаров
